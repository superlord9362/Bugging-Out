...
public class MoldyCeilingFeature extends Feature<NoneFeatureConfiguration> {
	public MoldyCeilingFeature(Codec<NoneFeatureConfiguration> codec) {
		super(codec);
	}
				...
				if (random.nextInt(5) == 0) {
					for(Direction direction : Direction.Plane.VERTICAL) {
						if (world.getBlockState(blockPos.relative(direction)).isFaceSturdy(world, blockPos.relative(direction), direction)) {
							world.setBlock(blockPos, BOBlocks.MOLD_CARPET.get().defaultBlockState().setValue(MoldBlock.getFaceProperty(direction), Boolean.valueOf(true)), 0);
							flag = true;
