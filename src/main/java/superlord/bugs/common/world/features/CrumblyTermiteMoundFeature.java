...
public class CrumblyTermiteMoundFeature extends Feature<BlockStateConfiguration> {
	public CrumblyTermiteMoundFeature(Codec<BlockStateConfiguration> p_66017_) {
		super(p_66017_);
	}
		...
		for(int k = -i; k < i; ++k) {
			for(int l = -i; l < i; ++l) {
				...
				if (d0 < 0.0D) {
					...
					if (isIcebergState(blockstate) || blockstate.is(BOBlocks.FERROUS_TERMOSTONE.get())) {
						if (p_66040_) {
							this.setBlock(p_66039_, blockpos, BOBlocks.POROUS_TERMOSTONE.get().defaultBlockState());
						} else {
	...
	private void setIcebergBlock(BlockPos p_66086_, LevelAccessor p_66087_, Random p_66088_, int p_66089_, int p_66090_, boolean p_66091_, boolean p_66092_, BlockState p_66093_) {
		...
		if (blockstate.getMaterial() == Material.AIR || blockstate.is(BOBlocks.FERROUS_TERMOSTONE.get())) {
			...
			if (p_66092_ && !blockstate.is(Blocks.WATER) && (double)p_66089_ <= (double)p_66088_.nextInt(Math.max(1, p_66090_ / i)) + (double)p_66090_ * 0.6D && flag) {
				this.setBlock(p_66087_, p_66086_, BOBlocks.CRUMBLY_TERMOSTONE.get().defaultBlockState());
			} else {
	...
	private static boolean isIcebergState(BlockState p_159886_) {
		return p_159886_.is(BOBlocks.TERMOSTONE.get()) || p_159886_.is(BOBlocks.CRUMBLY_TERMOSTONE.get()) || p_159886_.is(BOBlocks.FERROUS_TERMOSTONE.get()) || p_159886_.is(BOBlocks.POROUS_TERMOSTONE.get());
	}
		...
		for(int j = -i; j <= i; ++j) {
			for(int k = -i; k <= i; ++k) {
				for(int l = 0; l <= p_66055_; ++l) {
					...
					if (isIcebergState(blockstate) || blockstate.is(BOBlocks.CRUMBLY_TERMOSTONE.get())) {
